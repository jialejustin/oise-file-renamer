...
DROPDOWN_OPTIONS.append("➕ Manual Entry / Not Found...")
...
if uploaded_files:
    ...
    with col_preview:
        ...
        if ext == "pdf":
            ...
            st.info("📂 Word Document: Download to verify content.")
            st.download_button("⬇️ Download Original", data=current_file, file_name=current_file.name)
    ...
    with col_controls:
        ...
        c1, c2 = st.columns(2)
        with c1:
            if st.button("⬅️ Previous", disabled=st.session_state.file_idx == 0, use_container_width=True):
